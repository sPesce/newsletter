...
def calculate_recipients
  ...
  SUBSCRIBERS - UNSUBSCRIBED #using set notation, removed "UNSUBSCRIBED" set from "SUBSCRIBERS"
end
...
def print_recipients
  ...
  recip = calculate_recipients
  #Shovel each recipient from array into a string, add delimiter (i.e. comma and space)
  #Remove trailing comma and space
  recip = recip.reduce('') {|memo, recipient| memo << (recipient + ", ")}.chomp(', ')
  p recip
end
...
def print_one_article(article)
  ...
  piece = ARTICLES[article]
  puts piece[:title]
  puts "by: #{piece[:author]}"
  puts piece[:text]
  puts ''
end
